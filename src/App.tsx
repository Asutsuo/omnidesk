...
import { emptyData, LIMITS, type AppData, type Assignment, type Checklist, type Flashcard, type Profile, type StudyResource, type Subject, type Team, type TimerState } from "./data";
...
function App() {
  ...
  const runningTimer = data.timers.find((item) => item.status === "running");
  const floatingTimer = <FloatingTimer data={data} timer={runningTimer} onUpdate={timerUpdate} onDelete={timerDelete} onComplete={timerComplete} onOpen={(timer) => { if (timer.scope === "subject" && timer.subjectId && data.subjects.some((item) => item.id === timer.subjectId)) setSubjectId(timer.subjectId); else navigate("timer"); }} />;
