import { Eye, GripVertical, Pause, Timer as TimerIcon, Trash2, X } from "lucide-react";
...
type Props = { data: AppData; timer?: TimerState; onUpdate: (timer: TimerState) => void; onDelete: (id: string) => void; onComplete: (timer: TimerState, seconds: number) => void; onOpen: (timer: TimerState) => void };
...
function FloatingTimer({ data, timer, onUpdate, onDelete, onComplete, onOpen }: Props) {
  ...
  return <aside className={`timer-widget ${position.side}`} style={style} onPointerMove={move} onPointerUp={release}>
    ...
    <div className="timer-widget-actions"><button onClick={() => onUpdate(pauseTimer(timer))} title="Pausar"><Pause /></button><button onClick={() => setHidden(true)} title="Ocultar"><X /></button><button className="danger" onClick={() => window.confirm("Excluir este relógio ativo?") && onDelete(timer.id)} title="Excluir"><Trash2 /></button></div>
  </aside>;
}
