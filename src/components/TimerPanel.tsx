...
function TimerPanel({ scope, subjectId, timer, label, onStart, onUpdate, onDelete, onComplete }: Props) {
  ...
  const reset = () => { saveCurrentProgress(); const paused = pauseTimer(current); const duration = current.type === "pomodoro" ? current.focusMinutes * 60 : current.durationSeconds; onUpdate({ ...paused, mode: "focus", durationSeconds: duration, status: "paused", startedAt: null, elapsedSeconds: 0, recordedSeconds: 0, completedCycles: 0, remainingSeconds: current.type === "pomodoro" ? duration : 0, updatedAt: new Date().toISOString() }); };
  ...
  const configureCycles = (cycles: number) => { const cyclesTarget = Math.min(LIMITS.pomodoroCycles, Math.max(1, cycles || 1)); onUpdate({ ...current, cyclesTarget, completedCycles: Math.min(current.completedCycles, cyclesTarget), status: "paused", startedAt: null, updatedAt: new Date().toISOString() }); };
  ...
  const sequenceDone = current.type === "pomodoro" && current.completedCycles >= current.cyclesTarget;
  return <section className="timer-workspace"><div className="timer-kind-switch"><button className={current.type === "pomodoro" ? "active" : ""} onClick={() => changeType("pomodoro")}>Pomodoro</button><button className={current.type === "stopwatch" ? "active" : ""} onClick={() => changeType("stopwatch")}>Cronômetro</button></div>{label && <div className="timer-context"><span style={{ background: "currentColor" }} />{label}</div>}<div className="timer-ring large" style={{ "--timer-progress": `${progress}deg` } as React.CSSProperties}><div><span className="timer-state">{sequenceDone ? "SESSÃO CONCLUÍDA" : current.status === "running" ? "EM ANDAMENTO" : "PAUSADO"}</span><strong>{formatTimer(seconds, current.type === "stopwatch")}</strong>{current.type === "pomodoro" && <small>{current.mode === "focus" ? `Ciclo de foco · ${current.focusMinutes} min` : `Intervalo · ${current.breakMinutes} min`}</small>}</div></div>{current.type === "pomodoro" && <div className="pomodoro-cycles" aria-label={`${current.completedCycles} de ${current.cyclesTarget} ciclos concluídos`}><div className="pomodoro-cycle-marks">{Array.from({ length: current.cyclesTarget }, (_, index) => <span key={index} className={index < current.completedCycles ? "done" : index === current.completedCycles && !sequenceDone ? "current" : ""} />)}</div><small>{current.completedCycles}/{current.cyclesTarget} ciclos</small></div>}<div className="timer-controls"><button className="icon-button" onClick={reset} aria-label="Reiniciar"><RotateCcw /></button><button className="timer-main" disabled={sequenceDone} onClick={toggle} aria-label={current.status === "running" ? "Pausar" : "Iniciar"}>{current.status === "running" ? <Pause fill="currentColor" /> : <Play fill="currentColor" />}</button>{current.type === "pomodoro" ? <button className="icon-button" disabled={sequenceDone} onClick={nextPomodoro} aria-label="Pular etapa"><SkipForward /></button> : <button className="icon-button danger" onClick={deleteTimer} aria-label="Excluir cronômetro"><Trash2 /></button>}</div>{current.type === "pomodoro" && current.status === "paused" && <div className="timer-settings"><label>Foco<input type="number" min="5" max="180" value={current.focusMinutes} onChange={(event) => configure("focusMinutes", Number(event.target.value))} /></label><label>Intervalo<input type="number" min="1" max="60" value={current.breakMinutes} onChange={(event) => configure("breakMinutes", Number(event.target.value))} /></label><label>Ciclos<input type="number" min="1" max={LIMITS.pomodoroCycles} value={current.cyclesTarget} onChange={(event) => configureCycles(Number(event.target.value))} /></label></div>}{timer && <button className="timer-delete-link" onClick={deleteTimer}><Trash2 size={14} /> Excluir relógio salvo</button>}</section>;
}
