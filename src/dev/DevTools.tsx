...
function DevTools({ data, onApply }: Props) {
  ...
  return <aside className={`dev-tools ${open ? "open" : ""}`} aria-label="Ferramentas de demonstração">
    ...
    {open && <div className="dev-panel">
      ...
      <div className="dev-preview"><strong>Prévia</strong><span>{preview.subjects.length} matérias</span><span>{preview.assignments.length} trabalhos</span><span>{preview.checklistItems.length} itens</span><span>{preview.flashcards.length} cartões</span><span>{preview.resources.length} materiais</span><span>{preview.stats.length} registros</span></div>
