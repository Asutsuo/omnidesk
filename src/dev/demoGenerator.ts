...
const createQuestionDemo = (subjects: AppData["subjects"], prefix: string, profile: DemoContentProfile) => {
  if (!subjects.length || profile === "empty") return { questions: [], simulations: [], simulationAttempts: [] };
  const count = profile === "stress" ? Math.min(500, subjects.length * 30) : profile === "light" ? 12 : Math.max(30, subjects.length * 8); const stamp = new Date().toISOString();
  const questionBank = Array.from({ length: count }, (_, index) => ({ id: `${prefix}-question-${index}`, subjectId: subjects[index % subjects.length].id, collection: profile === "contest" ? `Prova anterior ${2022 + index % 4}` : "Lista de revisão", categories: [index % 2 ? "Aplicação" : "Fundamentos"], statement: `${questions[index % questions.length]} (${subjects[index % subjects.length].title})`, alternatives: [{ id: "A", text: "Uma interpretação parcialmente correta." }, { id: "B", text: "A alternativa correta conforme o conteúdo estudado." }, { id: "C", text: "Uma exceção que não se aplica ao caso." }, { id: "D", text: "Uma conclusão incompatível com o enunciado." }], correctAlternativeId: "B", explanation: "A alternativa B reúne os elementos exigidos pelo enunciado.", institution: profile === "contest" ? "Banca examinadora" : "OmniDesk", year: 2025, source: "Material de demonstração", createdAt: stamp, updatedAt: stamp }));
  const simulation = { id: `${prefix}-simulation-0`, title: profile === "contest" ? "Simulado geral do edital" : "Revisão geral", questionIds: questionBank.slice(0, 20).map((item) => item.id), shuffleQuestions: true, shuffleAlternatives: true, passingScore: 5, createdAt: stamp, updatedAt: stamp };
  const attempt = { id: `${prefix}-attempt-0`, simulationId: simulation.id, title: simulation.title, questions: questionBank.slice(0, 20), answers: Object.fromEntries(questionBank.slice(0, 20).map((item, index) => [item.id, index % 5 ? "B" : "A"])), passingScore: 5, status: "completed" as const, startedAt: stamp, completedAt: stamp, score: 8, passed: true };
  return { questions: questionBank, simulations: [simulation], simulationAttempts: profile === "light" ? [] : [attempt] };
};
...
export function generateDemoData(raw: DemoOptions, namespace = "preview"): AppData {
  ...
  if (options.profile === "empty") return normalizeData({ version: 7, onboarded: true, profile: { name: "Daniel", course: "Meu espaço de estudos", objective: "", weeklyGoal: 5 }, theme: "omnidesk" });
  ...
  const resources = subjects.slice(0, Math.min(subjects.length, 12)).flatMap((subject, index) => [{ id: id(`resource-${index}`, 0), subjectId: subject.id, title: `Videoaulas de ${subject.title}`, url: `https://www.youtube.com/results?search_query=${encodeURIComponent(subject.title)}`, type: "video" as const, description: "Referência para revisar aulas e encontrar explicações complementares.", tags: ["videoaula", "revisão"], collection: "Vídeos de apoio", createdAt: shift(-index).toISOString(), updatedAt: shift(-index).toISOString() }, { id: id(`resource-${index}`, 1), subjectId: subject.id, title: `Materiais de ${subject.title}`, url: `https://www.google.com/search?q=${encodeURIComponent(`${subject.title} material de estudo`)}`, type: "link" as const, description: "Ponto de partida para pesquisas e materiais complementares.", tags: ["pesquisa"], collection: "Referências", createdAt: shift(-index).toISOString(), updatedAt: shift(-index).toISOString() }]);
  const noteGroupId = id("note-group", 0); const noteGroups = [{ id: noteGroupId, name: "Rotina de estudo", order: 0, primaryNoteId: id("quick-note", 0) }];
  const quickNotes = [{ id: id("quick-note", 0), title: "Checklist de estudo", content: "# 📚 Checklist de estudo\n\n> Só considere o conteúdo concluído depois de cumprir todas as etapas.\n\n## 🧠 Aprendizado\n\n- [x] Estudar a teoria\n- [ ] Fazer recuperação ativa\n- [ ] Anotar os conceitos principais\n\n## ✍️ Prática\n\n- [ ] Resolver pelo menos 15 questões\n- [ ] Conferir o percentual de acertos\n\n### 🎯 Meta mínima: **70% de acertos**", groupId: noteGroupId, color: "#e9a070", order: 0, pinned: true, archived: false, createdAt: shift(-8).toISOString(), updatedAt: shift(-1).toISOString() }, { id: id("quick-note", 1), title: "Pontos para a próxima revisão", content: "## Lembretes\n\n- Revisar os erros do último simulado\n- Atualizar os flashcards difíceis\n- Separar o material da próxima semana\n\n> Priorizar os assuntos com menor percentual de acerto.", groupId: noteGroupId, color: "#7ea3b0", order: 1, pinned: false, archived: false, createdAt: shift(-5).toISOString(), updatedAt: shift(-2).toISOString() }];
  const scheduleEntries = subjects.slice(0, Math.min(subjects.length, options.profile === "light" ? 3 : 10)).map((subject, index) => ({ id: `${prefix}-schedule-${index}`, title: index % 3 === 1 ? `Revisão de ${subject.title}` : `Estudo de ${subject.title}`, description: "Bloco planejado para foco e revisão do conteúdo.", day: index % 5, startTime: `${String(8 + index % 6).padStart(2, "0")}:00`, endTime: `${String(9 + index % 6).padStart(2, "0")}:00`, category: index % 3 === 1 ? "review" as const : "study" as const, subjectId: subject.id, createdAt: shift(-10).toISOString(), updatedAt: shift(-1).toISOString() }));
  const currentStreak = options.profile === "light" ? 4 : options.profile === "contest" ? 18 : options.profile === "college" ? 9 : 42;
  const accessStreak = { current: currentStreak, best: currentStreak + (options.profile === "stress" ? 19 : 6), lastVisitDate: dateKey(new Date()) };
  return normalizeData({ version: 7, onboarded: true, profile: { name: "Daniel", course: catalog.course, objective: catalog.objective, weeklyGoal: options.profile === "light" ? 5 : 12 }, subjects, assignments, checklists, checklistSections, checklistItems, flashcards, notebooks, notes, timers, stats, teams, resources, scheduleEntries, quickNotes, noteGroups, noteWorkspace: { activeNoteId: quickNotes[0].id, selectedGroupId: noteGroupId, sort: "manual" }, accessStreak, ...createQuestionDemo(subjects, prefix, options.profile), focusMinutes: stats.reduce((sum, entry) => sum + entry.focusedSeconds / 60, 0), subjectView: "grid", homeShortcuts: ["subjects", "prazos", "notes", "schedule"], theme: options.profile === "college" ? "aurora" : "atlantic" });
}

export function mergeDemoData(current: AppData, demo: AppData): AppData {
  return normalizeData({ ...current, onboarded: true, subjects: [...current.subjects, ...demo.subjects], assignments: [...current.assignments, ...demo.assignments], checklists: [...current.checklists, ...demo.checklists], checklistSections: [...current.checklistSections, ...demo.checklistSections], checklistItems: [...current.checklistItems, ...demo.checklistItems], flashcards: [...current.flashcards, ...demo.flashcards], notebooks: [...current.notebooks, ...demo.notebooks], notes: [...current.notes, ...demo.notes], timers: [...current.timers, ...demo.timers.filter((timer) => timer.scope === "subject")], stats: [...current.stats, ...demo.stats], teams: [...current.teams, ...demo.teams], resources: [...current.resources, ...demo.resources], scheduleEntries: [...current.scheduleEntries, ...demo.scheduleEntries], quickNotes: [...current.quickNotes, ...demo.quickNotes], noteGroups: [...current.noteGroups, ...demo.noteGroups], questions: [...current.questions, ...demo.questions], simulations: [...current.simulations, ...demo.simulations], simulationAttempts: [...current.simulationAttempts, ...demo.simulationAttempts], focusMinutes: current.focusMinutes + demo.focusMinutes });
}
