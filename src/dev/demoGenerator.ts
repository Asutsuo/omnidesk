...
export function generateDemoData(raw: DemoOptions, namespace = "preview"): AppData {
  ...
  if (options.profile === "empty") return normalizeData({ version: 5, onboarded: true, profile: { name: "Daniel", course: "Meu espaço de estudos", objective: "", weeklyGoal: 5 }, theme: "omnidesk" });
  ...
  return normalizeData({ version: 5, onboarded: true, profile: { name: "Daniel", course: catalog.course, objective: catalog.objective, weeklyGoal: options.profile === "light" ? 5 : 12 }, subjects, assignments, checklists, checklistSections, checklistItems, flashcards, notebooks, notes, timers, stats, teams, resources, ...createQuestionDemo(subjects, prefix, options.profile), focusMinutes: stats.reduce((sum, entry) => sum + entry.focusedSeconds / 60, 0), subjectView: "grid", homeShortcuts: ["subjects", "prazos", "checklists", "estatisticas"], theme: options.profile === "college" ? "aurora" : "atlantic" });
}
